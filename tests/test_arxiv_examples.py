...
@pytest.fixture(params=["loads", "load"])
def parser(tmp_file_path_factory, request):
    if request.param == "loads":
        yield qiskit_qasm2.loads
    else:
        with tmp_file_path_factory() as path:

            def load(string):
                with open(path, "w", encoding="utf-8") as fp:
                    fp.write(string)
                return qiskit_qasm2.load(str(path.absolute()))

            yield load


def test_teleportation(parser):
    example = """\
...
def test_ripple_carry_adder(parser):
    example = """\
...
def test_randomised_benchmarking(parser):
    example = """\
...
def test_process_tomography(parser):
    example = """\
OPENQASM 2.0;
include "qelib1.inc";
...
def test_error_correction(parser):
    example = """\
